...
uniform float minThreshold, maxThreshold;
...
void disintegration(inout vec4 result) {
    ...
    /*
     ...
     bvec2 toDiscard = greaterThan( fract(fs_in.vTexCoord * scale), vec2(0.2f, 0.2f) );
     if( all(toDiscard) ) discard;
    
     // Mix textures
     //vec4 texColor = mix(mainTexColour, noiseTexColour, noiseTexColour.a); // blend textures
     result = texColor;
     */
    
     // Discard 2
    //if (noiseTexColour.a < minThreshold ) discard;
    
    if (material.bUseColor == false) {
         // Discard 3
        // If the value is outside the threshold, discard
        if( noiseTexColour.a < minThreshold ) discard;
        if( noiseTexColour.a > maxThreshold ) discard;
        
    } else {
        // Discard 4
        if( noiseTexColour.g > maxThreshold ) result = material.color;
    }
